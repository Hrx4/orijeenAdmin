...
const AllTeacher = () =>{
    ...
    const handleContactTable = async () => {
      ...
      const handleDelete = async(id) =>{
        ...
        try {
          const response = await fetch(`https://orijeen-main.vercel.app/teacher/${id}/`, {
            method: "DELETE",
            headers: {
              Accept: 'application/json',
              'Content-Type': 'application/json',
    ...
    return(
      ...
      <thead>
        ...
        <th style={{border:"1px solid black", padding:5}}>Education</th>
        <th style={{border:"1px solid black", padding:5}}>Address</th>
        <th style={{border:"1px solid black", padding:5}}>Salary</th>
        <th style={{border:"1px solid black", padding:5}}>Date of Job</th>
      ...
      <tbody>
        {
            teacherData?.map((item , index)=>(
                <tr key={item._id}>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherName}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherAge}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherGender}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherEducation}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherAddress}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherSalary}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherDoj}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherSubject}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherClass}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.teacherCourse}</td>
