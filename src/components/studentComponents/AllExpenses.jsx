  ...
  const [expenseData, setExpenseData] = useState([])
    const handleContactTable = async () => {
      ...
      const handleDelete = async(id) =>{
        ...
        try {
          const response = await fetch(`https://orijeen-main.vercel.app/expense/${id}/`, {
            method: "DELETE",
            headers: {
              Accept: 'application/json',
              'Content-Type': 'application/json',
      ...
      <tbody>
        {
          expenseData?.map((item , index)=>(
            <tr>
        <td style={{border:"1px solid black", padding:5}}>{item?.expenseTitle}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.expenseDescription}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.expenseAmount}</td>
        <td style={{border:"1px solid black", padding:5}}>{item?.expenseDate}</td>
