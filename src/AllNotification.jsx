...
const AllNotification = () => {
    ...
    const handleContactTable = async () => {
        ...
        try {
          const response = await fetch(`https://orijeen-main.vercel.app/notification/`, {
            method: "GET",
            headers: {
      ...
      const handleDelete = async(id) =>{
        ...
        try {
          const response = await fetch(`https://orijeen-main.vercel.app/notification/${id}/`, {
            method: "DELETE",
            headers: {
              Accept: 'application/json',
              'Content-Type': 'application/json',
       ...
       {notificationData?.map(
        (item) => (
          ...
          <tr style={{border:"1px solid black", padding:5}} key={item._id} onClick={()=>{console.log(item._id)}}>
              ...
              {item?.notifiactionTitle}
              </td>
              <td style={{border:"1px solid black" , padding:5}}>
              {item?.notifiactionDetails}
              </td>
              <td style={{border:"1px solid black" , padding:5}}>
              {item?.notifiactionLink}
              </td>
              <td style={{border:"1px solid black" , padding:5}}>
